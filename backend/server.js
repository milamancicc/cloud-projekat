...
//Request/GET all movies and data
app.get("/media", function (req, res) {
  console.log("ALL REQUEST");
  db.all(
    `SELECT id, name, year, genre, poster, description FROM media`,
...
//Middleware parsing JSON post
app.use(express.json());

//Adding post request to database, and sending back data
app.post("/media", function(req, res) {
  // console.log("ADD REQUEST");
  const { body } = req;
  if(!body.name || !body.year || !body.poster || !body.description){
  ...
  db.run(`INSERT INTO media (name, year, genre, poster, description) VALUES (?,?,?,?,?)`,
    ...
    function(err){
      ...
      db.all(
        `SELECT id, name, year, genre, poster, description FROM media WHERE id=last_insert_rowid()`, 
        function(err, response) {
          if(err){
            return res.status(500).send();
          }
...
//Changing movie data with post request (A3 didn't allow PUT to change data)
app.put("/media/:id", function(req, res) {
  console.log("CHANGE REQUEST");
  const { body } = req;
  if(!body.name || !body.year || !body.genre || !body.poster || !body.description){
    return res.status(400).send();
  }
...
app.delete("/media", function(req, res) {
  ...
  //not sure if we should delete the first two images?? if we should then use the commented line instead of the first one
  db.run("DELETE FROM media", function(err){
  //db.run("DELETE FROM media WHERE id > 2", function(err, res){
    if (err){
      return res.status(500).send();
    }
...
app.get("/media/:id", function(req, res) {
  ...
  db.all(`SELECT name, year, genre, poster, description FROM media WHERE id=?`,
    [req.params.id], 
    function(err, response) {
      if(err){
        return res.status(500).send();
